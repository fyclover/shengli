...
class OrderBase extends Base
{
    ...
    public function get_config(string $name)
    {
        ...
        if ($data['code'] != 200) return show($data);
        return $data['data'];
    }
    ...
    public function order_current_record()
    {
        ...
        if ($table_id <= 0) show([]);
        $list = GameRecords::where(['user_id' => self::$user['id'], 'table_id' => $table_id, 'close_status' => 1])
            ->field('bet_amt,game_peilv_id')
            ->whereTime('created_at', '-10 Minutes')
            ->select();

        show(['is_exempt' => 0, 'record_list' => $list]);
    }

    //限制用户投注金额
    protected function limit_money(int $money)
    {
        $limit = [20,50,100,500,1000,5000,10000,20000,50000,100000,200000,1000000,5000000,10000000,20000000,50000000,100000000,200000000,500000000,1000000000];
        $true = in_array($money,$limit);
        if (!$true) show([], config('ToConfig.http_code.error'), 'network connection error');
    }
}