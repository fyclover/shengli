...
class Order extends OrderBase
{
    ...
    public function user_bet_order(): string
    {
        ...
        if ($game_type != 2) show([], config('ToConfig.http_code.error'), 'game not selected');    //  为空，则提示
        //查询台座状态
        ...
        $user_money = 0;//用户金额。如果是没有下注过就是 用户钱包金额，下注过就需要加上当前这注的金额
        // 游戏当前 最大赔率限制 判断
        if ($delBetMoney > 0) {
        ...
        $data = array();
        //获取用户当前金额
        //$user_info = self::$user['money_balance'];
        ...
        try {
            foreach ($data as $key => $value) {
                //插入下单数据
                $id = Db::name('dianji_records')->insertGetId($value);
                //插入资金记录
                $value['source_id'] = $id;
        ...
        if ($save){
            worker_tcp(self::$user['id'],'当局投注',$data,203);
            show($data);
        };                                                 // 返回投注记录
