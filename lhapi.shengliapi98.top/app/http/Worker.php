...
//,$context
$worker = new Worker(env('worker.one','websocket://127.0.0.1:2002'));
...
$worker->onMessage = function ($connection, $data) {
    ...
    if(isset($data['code'])){
        ...
        if ($data['code'] == 205){
            $user_id .='_';
            $ret = sendMessageByUid($user_id, json_encode($array));// 返回推送结果
...
$worker->onWorkerStart = function ($worker) {
    ...
    $inner_text_worker = new Worker(env('worker.two','tcp://0.0.0.0:3002'));
